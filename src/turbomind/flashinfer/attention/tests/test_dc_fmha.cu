...
// Decoding stage configurations.  block_len=32 matches the FP16 PagedKv P32
// Context cubins shipped here; FP16 PagedKv P64 cubins are not in the set
// (lookup would assert "Trtllm-gen kernels not found").
//           name, head_num, kv_head_num, batch_size input_len, sequence_len, max_split_k, block_len
constexpr TestConfig Decoding_Small = {"Decoding_Small", 32, 8, 64, 1, 63, 128, 32};
constexpr TestConfig Decoding_Mid   = {"Decoding_Mid", 32, 8, 128, 1, 1023, 128, 32};
constexpr TestConfig Decoding_Large = {"Decoding_Large", 32, 8, 32, 1, 4093, 128, 32};
...
void processQAndKVUpdateForDecoding(thrust::universal_vector<T>&    dc_processed_q,  // 输出用于decoding attn的q数据
                                    ...
                                    const RopeKernelParam&          rope_param)
{
    // 前一次的Prefill阶段，将输入的prefill qkv，进行前处理，并将其写入到kv cache中
    {
        ...
        for (size_t b = 0; b < pf_batch_size; ++b) {
            for (size_t s = 0; s < kv_seq_len; ++s) {
                pf_token2batch_vec[pf_token_idx * 2 + 0] = b;
                pf_token2batch_vec[pf_token_idx * 2 + 1] = s;
                ++pf_token_idx;
            }
        }
    ...
    // 当前的Prefill阶段，将输入的prefill qkv，进行前处理，并将其写入到kv cache中
    {
        ...
        invokeDecodingQKNormRopeKVUpdate_(params);
        cudaDeviceSynchronize();
...
int test_trtllm_attention_impl(const TestConfig& config, int kQuantPolicy, int kTestIter = 10)
{
    ...
    if (kSequenceLen > 0) {
        ...
        processQAndKVUpdateForDecoding<Tkv>(dc_q,
                                            pf_processed_qkv,
                                            pf_qkv,
                                            dc_qkv,
                                            blocks,
    ...
    params.fmha.multi_block_mode = true;
...

    {
        ...
        std::cout << "\n>>> Compare k_cache (extracted from blocks) - New tokens (input_len=" << kInputLen
                  << " tokens)\n";
        // Layout: [B*H, S, D]，对比范围: [kSequenceLen, kSequenceLen + kInputLen)
        // 每个 batch*head 的数据是连续的，从 kSequenceLen * kHeadDim 开始，长度为 kInputLen * kHeadDim
        Compare(k_cache_extracted.data().get() + kSequenceLen * kHeadDim,  // 跳过历史tokens
                k_cache_ref.data().get() + kSequenceLen * kHeadDim,
                kInputLen * kHeadDim,    // 每个batch*head中新tokens的长度
                kContextLen * kHeadDim,  // stride: 每个batch*head的总长度
                kBatchSize * KvHeadNum,  // 总共的batch*head数量
                0);

        std::cout << "\n>>> Compare v_cache (extracted from blocks) - New tokens (input_len=" << kInputLen
                  << " tokens)\n";
        Compare(v_cache_extracted.data().get() + kSequenceLen * kHeadDim,
...
int main(int argc, char* argv[])
{
    ...
    if (mode == "full") {
        // Full test suite
        test_configs = {TestConfigs::Decoding_Small, TestConfigs::Decoding_Mid, TestConfigs::Decoding_Large};
    }
    else {
        // Default: run essential tests
        test_configs = {
            TestConfigs::Decoding_Small,
            TestConfigs::Decoding_Mid,
        };
    }
